...
let posts = [
 ...
 if (!localStorage.getItem("users")) {
    ...
    console.log("Danh sách users đã được lưu vào localStorage!");
} else {
    console.log("Users đã tồn tại trong localStorage!");
}
if (!localStorage.getItem("posts")) {
    localStorage.setItem("posts", JSON.stringify(posts));
    console.log("Danh sách posts đã được lưu vào localStorage!");
} else {
    console.log("posts đã tồn tại trong localStorage!");
}
...
function showTab1(){
    ...
    content.innerHTML = "";
    content.innerHTML += `
    <div id="tab1" class="tab-content"><div class="container mt-5">
...
document.getElementById('searchButton').addEventListener('click', function(){
   ...
   $filterUser.forEach($user => {
       ...
       listItem.textContent=`ID: ${$user.id}, ${$user.first_name} ${$user.last_name}, email: ${$user.email}`;
...
function showTab2(){
    ...
    const $postList = document.getElementById('postList');
        ...
        posts.forEach(post => {
            ...
            const listItem = document.createElement('li');
            listItem.className = 'list-group-item1';
            listItem.innerHTML = `
                <strong>ID:</strong> ${post.id} <br>
                <strong>Tiêu đề:</strong> ${post.title} <br>
                <strong>Ngày tạo:</strong> ${post.created_at} <br>
                <strong>Người tạo:</strong> ${$user ? `${$user.first_name} ${$user.last_name}` : 'Unknown'}
            `;
...
function showTab3(){
    ...
    content.innerHTML +=`
        ...
        <div class="mb-3">
            ...
            <input type="number" class="form-control" id="postIdInput">
        </div>
        <button id="viewPostButton" class="btn btn-primary">Xem chi tiết</button>
        ...
        <div id="postDetail" class="mt-4" style="display:none;">
            ...
            <p><strong>Ngày tạo:</strong> <span id="postCreatedAt"></span></p>
            <p><strong>Ngày sửa đổi:</strong> <span id="postUpdatedAt"></span></p>
            <p><strong>Người tạo:</strong> <span id="postAuthor"></span></p>
    ...
    document.getElementById('viewPostButton').addEventListener('click', function() {
        ...
        if (post) {
            ...
            document.getElementById('postCreatedAt').textContent = post.created_at;
            document.getElementById('postUpdatedAt').textContent = post.updated_at;
            document.getElementById('postAuthor').textContent = author ? `${author.first_name} ${author.last_name}` : 'Unknown';
            document.getElementById('postImage').src = post.image;
            postDetailDiv.style.display = 'block';
        } else {
            alert('Không tìm thấy bài viết với ID này.');
        }
    });
}
...
function showTab4(){
    ...
    <div class="container mt-5">
        ...
        <button id="searchPostsButton" class="btn btn-primary">Tìm kiếm</button>
    ...
    $tab3.classList.remove('active')
    $tab1.classList.remove('active')
    document.getElementById('searchPostsButton').addEventListener('click', function() {
        ...
        if (user) {
            ...
            if (userPosts.length > 0) {
                ...
                postList.innerHTML = '<li class="list-group-item">Người dùng này chưa có bài viết.</li>';
            }
        } else {
...
$tab4.addEventListener('click',showTab4);
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (loggedInUser) {
        ...
        btnLogin.textContent = user.first_name;
        btnLogin.classList.remove("btn-outline-light");
        btnLogin.classList.add("btn-success");

        btnSignup.textContent = "Đăng xuất";
        btnSignup.classList.remove("btn-warning");
        btnSignup.classList.add("btn-danger");
